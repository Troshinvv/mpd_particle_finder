...
//  std::string file_rec = "/home/mikhail/flow_calculator/cmake-build-debug/fhcal_efficiency_2022_07_14.root";
  ...
  for( const auto& name : histo_names ){
    ...
    for( int i=1; i<=n_bins; i++ ){
      ...
      vec_x.push_back(x);
      auto acceptance = h1_sig->GetHistogram()->Integral(i, n_bins+1);
      g1_sig_acc->SetPoint( i-1, x, acceptance );
      auto rejection = h1_bg->GetHistogram()->Integral(0, i);
    ...
    pic->Draw();
    pic->Save("/mnt/c/Users/Misha/CLionProjects/RISOVALKA/example/pfsimple/pictures/lambda_lower_edge/"+name, "png");
    book.AddPage(pic);
  }

  std::string save_name = "/mnt/c/Users/Misha/CLionProjects/RISOVALKA/example/pfsimple/pictures/lambda_lower_edge.pdf";
  book.SavePDF(save_name);
  book.SaveRoot("~/lower_edge.root");
}