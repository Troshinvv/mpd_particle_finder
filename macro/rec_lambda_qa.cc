void rec_lambda_qa(){
	...
	auto dd = d
      ...
      .Define( "daughter2_prat", "std::vector<float> prat; for( int i=0; i<daughter2_p.size();++i){ prat.push_back( daughter2_p.at(i) / candidate_p.at(i) ); } return prat;" )
          ...
          "  continue;\n"
          " }\n"
          " if( daughter1_chi2_prim.at(i) < 0.535 ){\n"
          "  good_candidate.push_back(0);\n"
          "  continue;\n"
          " }\n"
          " if( daughter2_cos.at(i) < 0.99795 ){\n"
          "  good_candidate.push_back(0);\n"
          "  continue;\n"
          " }\n"
          " if( candidate_cos_topo.at(i) < 0.990 ){\n"
          "   good_candidate.push_back(0);\n"
          "   continue;\n"
          " }\n"
	...
	hist1d.push_back( dd.Histo1D( { "h1_daughter1_chi2_prim", ";#chi^{2}_{prim}^{1}; counts", 200, 0.0, 2000.0 }, "daughter1_chi2_prim" ) );
	hist1d.push_back( dd.Histo1D( { "h1_daughter2_chi2_prim", ";#chi^{2}_{prim}^{2}; counts", 200, 0.0, 2000.0 }, "daughter2_chi2_prim" ) );
	...
	for( auto cut : cuts ){
		hist1d.push_back( dd.Histo1D( { std::data("h1_mass_"+cut), ";m (GeV/c^2); counts", 100, 1.05, 1.25 }, "candidate_mass", cut ) );
			...
			hist1d.push_back( dd.Histo1D( { std::data("h1_daughter1_chi2_prim_"+cut), ";#chi^{2}_{prim}^{1}; counts", 200, 0.0, 2000.0 }, "daughter1_chi2_prim", cut ) );
      ...
      hist1d.push_back( dd.Histo1D( { std::data("h1_candidate_chi2_geo_"+cut), ";#chi^{2}_{geo}; counts", 200, 0.0, 100.0 }, "candidate_chi2_geo", cut ) );
			hist1d.push_back( dd.Histo1D( { std::data("h1_candidate_chi2_topo_"+cut), ";#chi^{2}_{topo}; counts", 150, 0.0, 500.0 }, "candidate_chi2_topo", cut ) );
			...
			hist1d.push_back( dd.Histo1D( { std::data("h1_daughter_dca_"+cut), ";DCA; counts", 100, 0.0, 5.0 }, "daughter_dca", cut ) );
