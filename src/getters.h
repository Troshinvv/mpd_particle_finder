...
/*inline auto GetPhiProt(const ROOT::RVec<OutputContainer>& candidates, std::vector<ROOT::Math::PtEtaPhiMVector> candidate_momenta,ROOT::VecOps::RVec<ROOT::Math::LorentzVector<ROOT::Math::PtEtaPhiE4D<double> > > global_mom ){
  std::vector<double> phiprot{};
		...
		ROOT::Math::Boost boost(-mom.BoostToCM());
		ROOT::Math::PxPyPzEVector boosted = boost(ROOT::Math::PxPyPzEVector(global_mom.at(daughter1).Px(),global_mom.at(daughter1).Py(),global_mom.at(daughter1).Pz(),global_mom.at(daughter1).E()));
		
                phiprot.push_back(boosted.Phi());
  }
  return phiprot;
}*/
inline auto GetMCPhiProt(const ROOT::RVec<OutputContainer>& candidates, std::vector<ROOT::Math::PtEtaPhiMVector> candidate_momenta,ROOT::VecOps::RVec<ROOT::Math::LorentzVector<ROOT::Math::PtEtaPhiE4D<double> > > global_mom, ROOT::VecOps::RVec<ROOT::Math::LorentzVector<ROOT::Math::PtEtaPhiE4D<double> > > sim_mom,ROOT::VecOps::RVec<int> sim_ids ){
  std::vector<double> phiprot{};
        for(int i=0;i<candidates.size();i++){
                const auto candidate = candidates.at(i);
                const auto mom = candidate_momenta.at(i);
		...
		if( daughter1 >= sim_ids.size() ){
			phiprot.push_back(-999);
      			continue;
    		}
		double mc_prot_idx = sim_ids.at(daughter1);
		const auto mc_mom_prot = sim_mom.at(mc_prot_idx);
		
                ROOT::Math::PxPyPzEVector boosted = boost_to_rest(ROOT::Math::PxPyPzEVector(mc_mom_prot.Px(),mc_mom_prot.Py(),mc_mom_prot.Pz(),mc_mom_prot.E()));
                phiprot.push_back(boosted.Phi());
  }
  return phiprot;
}
inline auto GetMCThetaProt(const ROOT::RVec<OutputContainer>& candidates, std::vector<ROOT::Math::PtEtaPhiMVector> candidate_momenta,ROOT::VecOps::RVec<ROOT::Math::LorentzVector<ROOT::Math::PtEtaPhiE4D<double> > > global_mom, ROOT::VecOps::RVec<ROOT::Math::LorentzVector<ROOT::Math::PtEtaPhiE4D<double> > > sim_mom,ROOT::VecOps::RVec<int> sim_ids ){
  std::vector<double> phiprot{};
        for(int i=0;i<candidates.size();i++){
                const auto candidate = candidates.at(i);
                const auto mom = candidate_momenta.at(i);
		...
		if( daughter1 >= sim_ids.size() ){
                ...
                ROOT::Math::PxPyPzEVector boosted = boost_to_rest(ROOT::Math::PxPyPzEVector(mc_mom_prot.Px(),mc_mom_prot.Py(),mc_mom_prot.Pz(),mc_mom_prot.E()));
                phiprot.push_back(boosted.Theta());
  }
  return phiprot;
}
...
inline auto GetThetaProt(const ROOT::RVec<OutputContainer>& candidates, std::vector<ROOT::Math::PtEtaPhiMVector> candidate_momenta,ROOT::VecOps::RVec<ROOT::Math::LorentzVector<ROOT::Math::PtEtaPhiE4D<double> > > global_mom, ROOT::VecOps::RVec<ROOT::Math::LorentzVector<ROOT::Math::PtEtaPhiE4D<double> > > sim_mom,ROOT::VecOps::RVec<int> sim_ids ){
  std::vector<double> phiprot{};
        for(int i=0;i<candidates.size();i++){
                const auto candidate = candidates.at(i);
                const auto mom = candidate_momenta.at(i);
...
inline auto GetTruePDG( const ROOT::RVec<OutputContainer>& candidates,
		  ...
		  ROOT::VecOps::RVec<int> simmatchmothid) try {
  ...
  for( const auto& cand : candidates ){
    ...
    if ( mother_id1 >= simmatchmothid.size() ){
      true_pdg.push_back(-993);
      continue;
    }
    if (simmatchmothid.at(mother_id1)<0){
        true_pdg.push_back(-992);
        continue;
    }
    ...
    true_pdg.push_back(sim_pid.at(simmatchmothid.at(mother_id1)));
  }
  return true_pdg;
...
inline auto GetTruepT( const ROOT::RVec<OutputContainer>& candidates,
  ...
  for( const auto& cand : candidates ){
   ...
   true_pdg.push_back(sim_pt.at(simmatchmothid.at(mother_id1)));
  }
  return true_pdg;
} catch ( const std::out_of_range &e ){
  std::cerr << __func__ << "(): Exception was issued" << "\n";
...
inline auto GetTrueId( const ROOT::RVec<OutputContainer>& candidates,
  ...
  for( const auto& cand : candidates ){
   ...
   if( mother_id1 != mother_id2 ){
    ...
    if (simmatchmothid.at(mother_id1)<0){
        true_id.push_back(-992);
        continue;
    }    
true_id.push_back(mother_ids.at(simmatchmothid.at(mother_id1)));
  }
  return true_id;
} catch ( const std::out_of_range &e ){
  std::cerr << __func__ << "(): Exception was issued" << "\n";
...
inline auto GetUniGenFlag( const ROOT::RVec<OutputContainer>& candidates,
                  ...
                  ROOT::VecOps::RVec<int> sim_ids,
                  ROOT::VecOps::RVec<int> sim_pid,
		  ROOT::VecOps::RVec<int> sim_uniflag,
		  ROOT::VecOps::RVec<int> simmatchmothid) try {
  std::vector<int> true_id;
  for( const auto& cand : candidates ){
    auto daughters_id = cand.GetDaughterIds();
   ...
   if (mother_id1 != -1 && mother_id2!=-1 && mother_id1==mother_id2){
	...
	continue;}
	if (simmatchmothid.at(mother_id1)<0){
        true_id.push_back(-50);//no part in vec
        continue;
    }
	if(sim_uniflag.at(simmatchmothid.at(mother_id1))==-3){
		true_id.push_back(-3);//original
		continue;}
	if(sim_uniflag.at(simmatchmothid.at(mother_id1))==-9){
                true_id.push_back(-9);//enhanced
		continue;}
	if(sim_uniflag.at(simmatchmothid.at(mother_id1))==-15){ 
                true_id.push_back(-15);//first in the events where is no origin
		continue;}
	if(sim_uniflag.at(simmatchmothid.at(mother_id1))==0){
		true_id.push_back(-25);//access empty map element
		continue;} 
	if(sim_uniflag.at(simmatchmothid.at(mother_id1))==-900){
    ...
    if (simmatchmothid.at(mother_id1)<0){
        true_id.push_back(-55);//no part in vec
        continue;
    }
